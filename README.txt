...
very much an alpha version and I plan on leaving it at that. It is also to
note that even though this project is very focussed on privacy (no server,
encryption) it is by no means production ready.
...
Remember that the CLI does not have proper sync features, it will attempt to
synchronize with every device found without proper authentication. This can
lead to malicious behaviour, i.e. a malicious actor with a version of the app
can take all your notes. Moreover, the CLI version does not remember device
names, forcing the user to type it in each time it logs in.
